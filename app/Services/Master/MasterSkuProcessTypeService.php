...
class MasterSkuProcessTypeService
{
    ...
    function edit(Request $request)
    {
        ...
        $data['category'] = strtoupper($data['category']);
        $oldData = MasterSkuProcessType::where('id', $request->id)->firstOrFail();
        
        $oldData->update($data);
    }

    public function get_all_name() {
        return DB::table('mst_sku_process_type')->select('id', 'description', 'flag_active')->where('flag_active', '=', 1)->distinct()->get();
    }
    ...
    public function pagination_sku_process_type(Request $request){
        ...
        // $query = MasterSkuProcessType::query()
        // ->where('flag_active', '=', 1);

        $recordsTotal = MasterSkuProcessType::where('flag_active', '=', value: 1)->count();
        ...
        if (!empty($search)) {
            $query->where(function ($q) use ($search) {
                ...
                ->orWhere("manual_id", 'like', "%$search%")
                ->orWhere("category", 'like', "%$search%")
                ->orWhere("item_type.description", "like","%$search%")
                ->orWhere("item_type.prefix", "like","%$search%")
